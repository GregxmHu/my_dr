...
class Transformer(nn.Module):
    ...
    def __init__(self, model_name_or_path: str, max_seq_length: Optional[int] = None,
                 ...
                 cache_folder: str = "/data/private/huxiaomeng/pretrained_models/",
                 **auto_model_kwargs):
        ...
        local_model_name=model_name_or_path.replace('/','-')
        cache_dir=os.path.join(cache_folder,local_model_name)
    ...
    def forward(self, features):
        ...
        if 't5' in self.model_name_or_path or 'T5' in self.model_name_or_path:
            batch_size=features['input_ids'].shape[0]
            decoder_input_ids=torch.zeros(batch_size,1,dtype=int).to(features['input_ids'].device)
            trans_features['decoder_input_ids']=decoder_input_ids
    ...
    def tokenize_bos_eos(self, to_tokenize):
        ...
        for seq, att in zip(out["input_ids"], out["attention_mask"]):
            ...
            new_seq.append(seq)
            new_att.append(att)
        #out['input_ids']=new_seq
        #out['attention_mask']=new_att
