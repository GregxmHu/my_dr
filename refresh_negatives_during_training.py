...
with open(qrels_path_list_file,'w') as f:
    pass
score_path="/data/home/scv0540/run/my_dr/scores/{}_{}-pooling_{}_train.txt".format(args.model,args.pooling,args.datasets)  
qrels_path_list=[]
    ...
    with open(checkpoint_path_list_file,'r') as f:
        ...
        for item in f:
            ...
            if (round_num,stage_num) not in checkpoint_path_list:
                ...
                os.system("srun --job-name=inference_{}_{}-pooling_{}_round{}-stage{} --nodes=1 --gpus=8 --mem=300G --exclusive bash inference.sh {} {} {} {} {} {}".format(args.model,args.pooling,args.datasets,round_num,stage_num,args.model,args.pooling,args.datasets,round_num,stage_num,"train") )
                # generate new qrels_file
                next_stage_qrels_path="{}/round{}-stage{}_qrels-irrels.train.tsv".format(qrels_dir)
                #read scores generated by up inferenced
                scores={}
                with open(score_path,'r') as f:
                    for item in tqdm(f,desc="load score files produced by {}".format(path),disable=False):
                        item=item.strip('\n').split('\t')
                        if item[3]=="dot_score":
                            continue
                ...
                with open (origin_qrels_path,'r') as f:
                    for item in tqdm(f,desc="generate new qrels for next stage ",disable=False):
